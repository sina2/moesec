...
binmode( STDOUT, ":utf8" );
    ...
    @ico_rank = <LST>;
    close(LST);
    foreach $buf (@ico_rank) { utf8::decode($buf) }
...
EOM
    ...
    foreach (@sort_rank) {
        if ( $ico_rank{$_} ) { ++$num; $all_num = $all_num + $ico_rank{$_}; }
    }
...
EOM
    ...
    foreach ( 0 .. $st_top ) {
        ...
        if ( !$per ) { $per = 1; }
        if ( $ico_rank{$st_tp} ) {
            $gif_w = int( $ico_rank{$st_tp} * $g_width );
            if ( !$gif_w ) { $gif_w = 1; }
            print
                "<tr><td>$ex_num</td><td><b>$ico_name{$st_tp}</b></td><td><b>$ico_rank{$st_tp}</b></td><td nowrap><img src=$icon_dir\graph.gif height=\"12\" width=\"$gif_w\"> $per\%</td></tr>";
        }
    }
...
binmode( STDOUT, ":utf8" );
    ...
    if ( $in{'del_submit'} ) {
        ...
        sysopen( RLT, "$i_rank_log", O_RDONLY )
        ...
        if ($fll) {
            ...
            #open(RLT,">$i_rank_log") || &error("Can't open $i_rank_log");
            sysopen( RLT, "$i_rank_log", O_WRONLY | O_TRUNC | O_CREAT )
                || &error("Can't open $i_rank_log");
            print RLT @new_ir;
            close(RLT);
        }
...
binmode( STDOUT, ":utf8" );
    ...
    if ( !$ck_ad_pwd || ( $in{'name'} ne $ad_name ) ) {
        &get_cookie;
        &set_cookie;
    }
    ...
    foreach (@reg_num) {
        ...
        $new
            = "$date$ic_nm\tico$date$ic_nm$tail\t$in{$upf_name}\t$cut_name\t$pwd\t\n";
        push( @new, $new );
    }
...
binmode( STDOUT, ":utf8" );
    ...
    print <<"EOM";
<br>
<center><font color=\"$t_color\" size=6 face=\"$t_face\"><b><SPAN>\"$title\"アイコンこ〜な〜</SPAN></b></font></center><br>
        ...
        if ( $in{i_pg} eq "all" ) { $st_num = 0; $ed_num = $ia_num - 1; }
            ...
            foreach ( 1 .. $ict_num ) {
                if ( $in{i_pg} eq "$_" ) {
                    ...
                    $ict_num_ex
                        = "$ict_num_ex <a href=\"$iconCGI?i_pg=$_&icon_edit=on&name=$pg_name&pwd=$in{pwd}&bg_img=$in{bg_img}\">$_</a>";
                }
            }

            $ict_num_ex
                = "<br>$ict_num_ex <a href=\"$iconCGI?i_pg=all&icon_edit=on&name=$pg_name&pwd=$in{pwd}&bg_img=$in{bg_img}\">all</a>";
        }
...
EOM
    ...
    foreach $Icon ( $st_num .. $ed_num ) {
        $i++;
        ( $num, $file, $name, $register, $pwd )
            = split( /\t/, @Icons[$Icon] );
        if ( !$ck_mpwd ) { $ck_pwd = &pwd_de("$pwd"); }
        if ( $num =~ /pri_ico/ ) {
            $name = "$name\　<font color=blue><b>（\t$register\t専用）</b></font>";
        }
        if (   ( ( $register eq $cut_name ) && ( $ck_mpwd || $ck_pwd ) )
            || ( $ck_ad_pwd && ( $in{'name'} eq $ad_name ) ) )
        {
            $ckbx = "<input type=checkbox name=del value=$num>";
        }
        else {
            $ckbx = "";
        }
...
binmode( STDOUT, ":utf8" );
    ...
    if ( !$in{'ad_num'} && !$in{'rst1_submit'} ) {
        &error("アイコン追加数が設定されてません");
    }
    if ( $in{'ad_num'} > $ico_rv_num ) {
        &error("一度に登録できるアイコンは$ico_rv_numコまでです");
    }
    $add_num = $in{'ad_num'};
...
EOM
    ...
    if ( $in{'rst1_submit'} ) {
        foreach (@rst_img) {
            ...
            print
                "<tr><td><img src=\"$img\"><input type=hidden name=del value=$num></td>";
            print
                "<td><input type=text name=\"icon_name$num\" value=\"$name\"></td>";
        }
    }
    else {
        foreach ( 1 .. $add_num ) {
...
binmode( STDOUT, ":utf8" );
    ...
    foreach (@in) {
        $_ =~ s/\s//g;
        if ( $_
            =~ /(.*)name=\"icon_file(\d*)\"(.*)filename=\"(.*)\"Content-Type:(.*)\/(.*)/
            )
        {
            $ic_nm    = $2;
            $f_name   = $4;
            ...
            if ( !$in{$upf_bin} ) {
                &error("$in{$upf_name}のアイコンファイルが選択されていません");
            }
            if ( length( $in{$upf_bin} ) > $ico_max ) {
                &error("$in{$upf_name}のアイコンファイルサイズが$max_sz\kを超えています");
            }
            ...
            if ( !$tail ) {
                &error("$in{$upf_name}のアイコンはアップロードできないファイル形式です $f_type");
            }
...
binmode( STDOUT, ":utf8" );
    ...
    $c_pwd    = $in{pwd};
    $cook
        = "name\:$c_name\,email\:$c_email\,url\:$c_url\,pwd\:$c_pwd\,icon\:$c_icon\,color\:$c_color\,mail_ex\:$c_m_ex";
